//
//  HKFSay_SayCell.swift
//  YueDongQuan
//
//  Created by HKF on 2016/11/8.
...
class HKFSay_SayCell: UITableViewCell,UITableViewDelegate,UITableViewDataSource {
    ...
    func updateCellLayoutConstraints(model:DiscoveryArray) {
        let layoutModel = model
        if layoutModel.address == ""{
            ...
            self.distanceLabel?.text = layoutModel.address
        }
        
        if layoutModel.images.count != 0 {
            let h1 = cellHeightByData1(layoutModel.images.count)
...
extension HKFSay_SayCell {
    ...
    private func cellHeightByData1(imageNum:Int)->CGFloat{
