...
class MyNotifySay_SayVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.view.addSubview(tableiewForMyNotify)
        tableiewForMyNotify.registerClass(HKFTableViewCell.self, forCellReuseIdentifier: "HKFTableViewCell")
        tableiewForMyNotify.backgroundColor = UIColor(red: 246/255, green: 246/255, blue: 246/255, alpha: 1)
        tableiewForMyNotify.separatorStyle = .None
        
        tableiewForMyNotify.dataSource = self
        tableiewForMyNotify.delegate = self
        
        
        //下拉
        tableiewForMyNotify.mj_header = MJRefreshNormalHeader(refreshingTarget: self, refreshingAction: #selector(MyNotifySay_SayVC.dropDownRef))
        //上拉
        tableiewForMyNotify.mj_footer = MJRefreshAutoNormalFooter(refreshingTarget: self, refreshingAction: #selector(MyNotifySay_SayVC.pullUpRef))
    }
...
extension MyNotifySay_SayVC : UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let model = self.myNotifyModelData[indexPath.section]
    ...
    func reloadCellHeightForModelAndAtIndexPath(model: DiscoveryArray, indexPath: NSIndexPath) {
        self.tableiewForMyNotify.reloadRowsAtIndexPaths([indexPath], withRowAnimation: UITableViewRowAnimation.Fade)
    }
}

//计算距离根据经纬度
extension MyNotifySay_SayVC {
