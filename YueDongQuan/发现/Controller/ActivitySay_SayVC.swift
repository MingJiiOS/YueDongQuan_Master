...
class ActivitySay_SayVC: UIViewController {
    
    private var tableiewForActivity = UITableView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight - 153), style: UITableViewStyle.Plain)
    ...
    override func viewDidLoad() {
        ...
        self.view.addSubview(tableiewForActivity)
        tableiewForActivity.registerClass(HKFTableViewCell.self, forCellReuseIdentifier: "HKFTableViewCell")
        tableiewForActivity.backgroundColor = UIColor(red: 246/255, green: 246/255, blue: 246/255, alpha: 1)
        tableiewForActivity.separatorStyle = .None
        
        tableiewForActivity.dataSource = self
        tableiewForActivity.delegate = self
        
        
        //下拉
        tableiewForActivity.mj_header = MJRefreshNormalHeader(refreshingTarget: self, refreshingAction: #selector(ActivitySay_SayVC.dropDownRef))
        //上拉
        tableiewForActivity.mj_footer = MJRefreshAutoNormalFooter(refreshingTarget: self, refreshingAction: #selector(ActivitySay_SayVC.pullUpRef))
    }
...
extension ActivitySay_SayVC : UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let model = self.activityModelData[indexPath.section]
    ...
    func reloadCellHeightForModelAndAtIndexPath(model: DiscoveryArray, indexPath: NSIndexPath) {
        self.tableiewForActivity.reloadRowsAtIndexPaths([indexPath], withRowAnimation: UITableViewRowAnimation.Fade)
    }
}


//计算距离根据经纬度
extension ActivitySay_SayVC {
