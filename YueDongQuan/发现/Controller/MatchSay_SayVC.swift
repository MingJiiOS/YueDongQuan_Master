...
class MatchSay_SayVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.view.addSubview(tableiewForMatch)
        tableiewForMatch.registerClass(HKFTableViewCell.self, forCellReuseIdentifier: "HKFTableViewCell")
        tableiewForMatch.backgroundColor = UIColor(red: 246/255, green: 246/255, blue: 246/255, alpha: 1)
        tableiewForMatch.separatorStyle = .None
        
        tableiewForMatch.dataSource = self
        tableiewForMatch.delegate = self
        
        
        //下拉
        tableiewForMatch.mj_header = MJRefreshNormalHeader(refreshingTarget: self, refreshingAction: #selector(MatchSay_SayVC.dropDownRef))
        //上拉
        tableiewForMatch.mj_footer = MJRefreshAutoNormalFooter(refreshingTarget: self, refreshingAction: #selector(MatchSay_SayVC.pullUpRef))
    }
...
extension MatchSay_SayVC : UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let model = self.matchModelData[indexPath.section]
    ...
    func reloadCellHeightForModelAndAtIndexPath(model: DiscoveryArray, indexPath: NSIndexPath) {
        self.tableiewForMatch.reloadRowsAtIndexPaths([indexPath], withRowAnimation: UITableViewRowAnimation.Fade)
    }
}


//计算距离根据经纬度
extension MatchSay_SayVC {
