...
class ImageSay_SayVC: UIViewController {
    
    private var tableiewForImage = UITableView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight - 153), style: UITableViewStyle.Plain)
    ...
    override func viewDidLoad() {
        ...
        self.view.addSubview(tableiewForImage)
        tableiewForImage.registerClass(HKFTableViewCell.self, forCellReuseIdentifier: "HKFTableViewCell")
        tableiewForImage.backgroundColor = UIColor(red: 246/255, green: 246/255, blue: 246/255, alpha: 1)
        tableiewForImage.separatorStyle = .None
        
        tableiewForImage.dataSource = self
        tableiewForImage.delegate = self
        
        
        //下拉
        tableiewForImage.mj_header = MJRefreshNormalHeader(refreshingTarget: self, refreshingAction: #selector(ImageSay_SayVC.dropDownRef))
        //上拉
        ...
        tableiewForImage.mj_footer = MJRefreshAutoNormalFooter(refreshingTarget: self, refreshingAction: #selector(ImageSay_SayVC.pullUpRef))
...
extension ImageSay_SayVC : UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let model = self.imageModelData[indexPath.section]
    ...
    func reloadCellHeightForModelAndAtIndexPath(model: DiscoveryArray, indexPath: NSIndexPath) {
        self.tableiewForImage.reloadRowsAtIndexPaths([indexPath], withRowAnimation: UITableViewRowAnimation.Fade)
    }
}

//计算距离根据经纬度
extension ImageSay_SayVC {
